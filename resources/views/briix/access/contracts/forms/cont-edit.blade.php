...
<!--Seleccion del usuario por defecto que tendra acceso al producto, personal previamente registrado y asociado a una empresa-->
                <div class="form-group">
                    ...
                    <div class="col-lg-10">
                        {{ Form::select('client_id', [''=>trans('validation.attributes.briix.access.contracts.client_id')]+ $clients,null, ['class' => 'form-control']) }}
                    </div><!--col-lg-10-->
                </div><!--form control-->
                ...
                <div class="form-group">
                    ...
                    <div class="col-lg-10">
                        {{ Form::select('executive_id',[''=>trans('validation.attributes.briix.access.contracts.executive_id')]+ $executives, null, ['class' => 'form-control']) }}
                    </div><!--col-lg-10-->
                </div><!--form control-->
                ...
                <div class="form-group">
                 ...
                 <div class="form-group">
                    ...
                    <div class="form-group">
                        {{ Form::label('plan', trans('validation.attributes.briix.access.contracts.associated_plans'), ['class' => 'col-lg-2 control-label']) }}

                        <div class="col-lg-3">
                            @if (count($plans) > 0)
                                @foreach($plans as $plan)
                                    <input type="checkbox" value="{{$plan->id}}" name="assignees_plans[]" {{in_array($plan->id, $user_plans) ? 'checked' : ''}} id="plan-{{$plan->id}}" /> <label for="plan-{{$plan->id}}">{{ $plan->name }}</label>
                                        <a href="#" data-role="plan_{{$plan->id}}" class="show-permissions small">
                                            (
                                    ...
                                    <br/>
                                    <div class="permission-list hidden" data-role="plan_{{$plan->id}}">
                                        @if ($plan->all)
                                            {{ trans('labels.briix.access.users.all_permissions') }}<br/><br/>
                                        @else
                                            @if (count($plan->permissions) > 0)
