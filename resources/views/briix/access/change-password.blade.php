...
@section ('title', trans('labels.briix.access.users.management') . ' | ' . trans('labels.briix.access.users.change_password'))

@section('page-header')
    <h1>
        {{ trans('labels.briix.access.users.management') }}
        <small>{{ trans('labels.briix.access.users.change_password') }}</small>
    </h1>
@endsection

@section('content')
    {{ Form::open(['route' => ['briix.access.user.change-password', $user], 'class' => 'form-horizontal', 'role' => 'form', 'method' => 'post']) }}

        <div class="box box-success">
            <div class="box-header with-border">
                <h3 class="box-title">{{ trans('labels.briix.access.users.change_password_for', ['user' => $user->name]) }}</h3>
